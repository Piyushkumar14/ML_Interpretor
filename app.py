...
def show_global_interpretation_shap(X_train, clf):
    ...
    # Generate the summary plot with updated matplotlib syntax
    shap.summary_plot(
        shap_values,
        X_train,
        plot_type="bar",
        max_display=5,
        plot_size=(12, 5),
        show=False,  # Disable immediate display to customize via Streamlit
    )

    # Display the plot with Streamlit
    st.pyplot(plt.gcf())

    # Clear the current figure to avoid overlapping in Streamlit
    plt.clf()
